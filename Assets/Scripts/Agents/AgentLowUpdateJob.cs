...
public struct AgentLowUpdateJob : IJobParallelForTransform
{
    ...
    public void Execute(int index, TransformAccess transform)
    {
        ...
        RotateTowards(ref transform, lookDir, finalNormal, rotationSpeeds[index], deltaTime);

        float3 forward = math.normalize(ProjectOnPlane(transform.rotation * new float3(0, 0, 1), finalNormal));
